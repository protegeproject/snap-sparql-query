...
public class ISBLANK_Evaluator_TestCase {
    ...
    public void shouldEvaluateIRIAsFalse() {
        EvaluationResult result = evaluator.evaluate(Collections.singletonList(new AtomicIRI(IRI.create("http://some.iri.com/A"))), SolutionMapping.emptyMapping(), mock(AlgebraEvaluationContext.class));
    ...
    public void shouldEvaluateMultiArgsAsError() {
        EvaluationResult result = evaluator.evaluate(Arrays.asList(blank, blank), SolutionMapping.emptyMapping(), mock(AlgebraEvaluationContext.class));
